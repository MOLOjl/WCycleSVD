...
void test5(){
    vector<int> shape_array1 = {512, 1024, 2048, 4096, 8192, 10240};
    ...
    for(int iter1=0; iter1<shape_array1.size(); iter1++){
        ...
        if(true)
        {
            ...
            for (int I=0; I<ITERS; ++I)
            {
                ...
                // printf("iter %d - last error: %s\n", I, cudaGetErrorString(cudaGetLastError()));
            }
            cusolv_time /= ITERS;
            our_time /= ITERS;
            printf("matrix:%d×%d×%d, speedup over cusolver: %lf/%lf = %lf\n", batch, height, width, cusolv_time, our_time, cusolv_time / our_time);
            // printf("matrix:%d×%d×%d, gemm time: %lf total time %lf\n", batch, height, width, cusolv_time, our_time);
            free(host_A);
            cudaFree(dev_A);
            cudaFree(dev_U);
            cudaFree(dev_V);
