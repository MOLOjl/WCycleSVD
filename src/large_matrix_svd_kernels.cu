...
#include "large_matrix_svd_kernels_tc.inc.cu"
...
#include "fused_summition_evd.inc.cu"
extern double wtime();

double EVD_(double *dev_jointG, double *dev_AiAj, double *dev_AiAi, double* dev_AjAj, double *dev_A, double *dev_V, int *dev_pairsOfEVD, int p, int q, int height, int width, int *dev_roundRobin, int batch, int k, int slice, int sliceNum, int iter)
{
    if(k>24 && gm_V==NULL){
    ...
    // {//manual check
    //     #define G_MAP(bt, pr, y, x) ((bt)*batch*4*k*k*p + (pr)*4*k*k + (y)*2*k + (x))
    //     #define AA_MAP(batch, pair, slice, y, x) ((batch)* k * k * sliceNum * p + (pair) * k * k * sliceNum + (slice) * k * k + (y) * k + (x))

    //     double *hst_G = new double[batch*4*k*k*p];
    //     double *hst_AiAi = new double[batch*k*k*p*sliceNum];
    //     cudaMemcpy(hst_G, dev_jointG, sizeof(double)*batch*4*k*k*p, cudaMemcpyDeviceToHost);
    //     cudaMemcpy(hst_AiAi, dev_AiAi, sizeof(double)*batch*sliceNum*k*k*p, cudaMemcpyDeviceToHost);
    //     for (int b=0; b<batch; ++b)
    //     for (int _p=0; _p < p; ++_p)
    //     {
    //         printf("batch %d pair %d:\n", b, _p);
    //         for (int y=0; y<k; y++)
    //         {
    //             for (int x=0; x<k; x++)
    //             {
    //                 double g_ele = hst_G[G_MAP(b,_p,y,x)];
    //                 printf("%.2f ", g_ele);
    //             }
    //             printf("\n");
    //         }
    ...
    //                 {
    //                     calc += hst_AiAi[AA_MAP(b,_p, s,y,x)];
    //                 }
    //                 printf("%.2f ", calc);
    //             }
    //             printf("\n");
    ...
    #else
        // printf("calling 1\n");
        myevd_fused_batched<<<dimGrid9, dimBlock9>>>(dev_AiAj, dev_AiAi, dev_AjAj, dev_jointG, dev_roundRobin, p, k, sliceNum);
    #endif
    }
    else if (k == 4)
    {
    ...
    #else
        // printf("calling 4\n");
        myevd_fused_batched_4<<<dimGrid9, dimBlock9>>>(dev_AiAj, dev_AiAi, dev_AjAj, dev_jointG, dev_roundRobin, p, k, sliceNum);
    #endif
    }
    else if (k == 8)
    {
    ...
    #else
        // printf("calling 8\n");
        myevd_fused_batched_8<<<dimGrid9, dimBlock9>>>(dev_AiAj, dev_AiAi, dev_AjAj, dev_jointG, dev_roundRobin, p, k, sliceNum);
    #endif
    }
    else if (k == 16)
    {	
    ...
    #else
        // printf("calling 16\n");
        // printf("dev_Aij=%lx, dev_Aii=%lx, dev_Ajj=%lx, dev_G=%lx, p=%d, k=%d, sliceNum=%d\n", 
        //        dev_AiAj, dev_AiAi, dev_AjAj, dev_jointG, p, k, sliceNum);
        myevd_fused_batched_16<<<dimGrid9, dimBlock9>>>(dev_AiAj, dev_AiAi, dev_AjAj, dev_jointG, dev_roundRobin, p, k, sliceNum);
    #endif
    }
    else if (k == 24)
    { 
    ...
    #else
        // printf("calling 24\n");
        myevd_fused_batched_24<<<dimGrid9, dimBlock9>>>(dev_AiAj, dev_AiAi, dev_AjAj, dev_jointG, dev_roundRobin, p, k, sliceNum);
    #endif
    }
    else
    {
    ...
    cudaDeviceSynchronize();
    auto end = wtime();
    if (auto errcode = cudaGetLastError()) 
    {
        printf("middle 2 - last error: %s\n", cudaGetErrorString(errcode));
        exit(-1);
    }
    ...
    if (k == 4)
    {
        // double *check_A, *check_V;
        // cudaMalloc(&check_A, height*width*batch*sizeof(double));
        // cudaMalloc(&check_V, width*width*batch*sizeof(double));
        // cudaMemcpy(check_A, dev_A, sizeof(double)*height*width*batch, cudaMemcpyDeviceToDevice);
        // cudaMemcpy(check_V, dev_V, sizeof(double)*width*width*batch, cudaMemcpyDeviceToDevice);
        ...
        // double *hst_ans = new double[max(height,width)*width*batch];
        // double *hst_res = new double[max(height,width)*width*batch];
    #else
        updateBlockColumn2_tc<4><<<dimGrid11, dim3(32,k/8,2)>>>(dev_A, dev_V, dev_jointG, dev_pairsOfEVD, p, q, height, width, slice);
    #endif
        // cudaDeviceSynchronize();
        // if (auto errcode = cudaGetLastError()) {  
        //     fprintf(stderr, "1: %s\n", cudaGetErrorString(errcode));
        ...
        // cudaMemcpy(hst_ans, dev_A, sizeof(double)*height*width*batch, cudaMemcpyDeviceToHost);
        // cudaMemcpy(hst_res, check_A, sizeof(double)*height*width*batch, cudaMemcpyDeviceToHost);

        // for ( int i=0; i < height*width*batch; ++i )
        // {
        //     if (fabs(hst_ans[i] - hst_res[i]) > 1e-5)
        //     {
        //         printf("false@1 [%d]: %f(x) <- %f\n", i, hst_res[i], hst_ans[i]);
        //         exit(0);
        //     } 
        // }

        // cudaMemcpy(hst_ans, dev_V, sizeof(double)*width*width*batch, cudaMemcpyDeviceToHost);
        // cudaMemcpy(hst_res, check_V, sizeof(double)*width*width*batch, cudaMemcpyDeviceToHost);

        // for ( int i=0; i < width*width*batch; ++i )
        // {
        //     if (fabs(hst_ans[i] - hst_res[i]) > 1e-5)
        //     {
        //         printf("false@2 [%d]: %f(x) <- %f\n", i, hst_res[i], hst_ans[i]);
        //         exit(0);
        //     } 
        // }
    ...
    #else
        updateBlockColumn2_tc<8><<<dimGrid11, dim3(32,k/8,2)>>>(dev_A, dev_V, dev_jointG, dev_pairsOfEVD, p, q, height, width, slice);
    #endif
    }
    else if (k == 16)
    {
    ...
    #else
        updateBlockColumn2_tc<16><<<dimGrid11, dim3(32,k/8,2)>>>(dev_A, dev_V, dev_jointG, dev_pairsOfEVD, p, q, height, width, slice);
    #endif
    }
    else if (k == 24)
    {
    ...
    #else
        updateBlockColumn2_tc<24><<<dimGrid11, dim3(32,k/8,2)>>>(dev_A, dev_V, dev_jointG, dev_pairsOfEVD, p, q, height, width, slice);
    #endif
        // cudaDeviceSynchronize();
        // if (auto errcode = cudaGetLastError()) {  
        //     fprintf(stderr, "1: %s\n", cudaGetErrorString(errcode));
