...
double svd_large_matrix(double* host_A, int* shape, double* host_diag, double* host_U, double* host_V, int dev_idx){
    int height0 = shape[1];
	int width0 = shape[2];
	int batch = shape[0];
	...
	// printf("matrix:%dx%dx%d, sweeps:%d, Total time: %lf(s)\n", batch, height, width, sweep, time_span.count());
